...
search_alg = "bfs" # use either bfs or dfs. the other tests will change accordingly.
...
def ind_test():
    ...
    with open("Current_Location.jpg", "rb") as f:
        assert len(f.read()) == secret_location_size
    ind_points += 5
...
def main():
    ...
    # check that they aren't hardcoding the password
    pass_matches = []
    for line in code.splitlines():
        stripped = line.strip()
        if not stripped.startswith("#"):  # skip comments
            matches_in_line = re.findall(r"\b\d{16}\b", stripped)
            pass_matches.extend(matches_in_line)
    if pass_matches:
        result["errors"].append("Make sure that you are not hardcoding the password in your code.")
        return result
